...
Lock dev behind Tailscale by applying a Traefik IP allow-list middleware to the dev resources only. The middleware itself ships in this directory at [`traefik-tailscale-only.yaml`](./traefik-tailscale-only.yaml) — deploy and reference it per the steps below.

**1. Install the middleware on the VPS:**
...
**2. Apply it to the dev resource (Coolify UI):**

In Coolify → the dev project → each dev application (web, stratum-api) → **Network** → **Custom Labels**, add:

```
traefik.http.routers.<auto-generated-router-name>.middlewares=tailscale-only@file
```

Find `<auto-generated-router-name>` by either:

- Inspecting the running container: `docker inspect <container-id> | jq '.[].Config.Labels'` and looking for the Coolify-generated `traefik.http.routers.*` keys, OR
- Visiting the Traefik dashboard if exposed.

Apply to **dev only** — prod stays open.

**3. Verify:**

```sh
# from your laptop, on Tailscale:
curl -I https://dev.hashden.app                  # → 200

# from anywhere not on Tailscale (e.g. a public-internet box, or your phone on cellular):
curl -I https://dev.hashden.app                  # → 403 Forbidden
```

If both succeed, the middleware isn't applied to that hostname's router. Re-check the label name matches what Coolify auto-generated.
